...
class XenCRMServer {
      ...
      return {
        tools: [
          ...
          },
          {
            ...
            description:
              'Create a new marketing campaign for an existing segment. Use this to launch campaigns to targeted customer groups.',
            inputSchema: {
              type: 'object',
              properties: {
                segment_id: {
                ...
                message_template: {
                  type: 'string',
                  description:
                    'Message template for the campaign (supports placeholders like {name}, {total_spend})',
                },
                campaign_type: {
                  type: 'string',
                  enum: [
                  ...
                  description:
                    'Type of campaign (optional, defaults to PROMOTIONAL)',
                },
                created_by: {
                  type: 'string',
          ...
          },
          {
            name: 'list_segments',
            description:
              'List all available customer segments that can be used for campaigns.',
            inputSchema: {
              type: 'object',
              properties: {
                limit: {
                  ...
                  maximum: 50,
                  description:
                    'Maximum number of segments to return (optional, defaults to 10)',
                },
              },
            },
          },
  ...
  private async handleCreateCampaign(args: any) {
    ...
    if (!segment_id || !name || !message_template || !created_by) {
      throw new Error(
        'Missing required fields: segment_id, name, message_template, and created_by are required'
      );
    }
    ...
    const responseText = [
      ...
      `• **Target Count:** ${
        segment?.preview_count || campaign.target_audience_count || 'Unknown'
      }`,
      ``,
      `**Message Template:**`,
  ...
  private async handleListSegments(args: any) {
  ...
  private async handleGetCampaignStats(args: any) {
    ...
    const responseText = [
      ...
      `**Delivery Statistics:**`,
      `• **Total Messages:** ${
        campaign.campaign_delivery_summary?.total_messages || 0
      }`,
      `• **Pending:** ${
        campaign.campaign_delivery_summary?.pending_count || 0
      }`,
      `• **Sent:** ${campaign.campaign_delivery_summary?.sent_count || 0}`,
      `• **Delivered:** ${
        campaign.campaign_delivery_summary?.delivered_count || 0
      }`,
      ...
      `• **Last Updated:** ${formatDate(
        campaign.campaign_stats?.last_updated || null
      )}`,
      ``,
      `**Target Segment:**`,
