...
export class DatabaseService {
    ...
    async createOrder(data) {
        try {
            ...
            if (data.customer_email && !customerId) {
                ...
                if (!customer) {
                    throw new Error(`Customer with email ${data.customer_email} not found`);
                }
    ...
    async updateCustomerStats(customerId, orderAmount) {
        try {
            ...
            const newTotalSpend = (customer.total_spend?.toNumber() || 0) + orderAmount;
    ...
    async createOutboxEvent(eventType, entityId, payload) {
        try {
    ...
    async getCustomerAnalytics(customerId) {
        try {
            if (customerId) {
                ...
                const totalSpent = customer.orders.reduce((sum, order) => sum + order.order_amount.toNumber(), 0);
    ...
    async getCampaignsBySegment(segmentId, limit = 10) {
        try {
    ...
    async getCampaignAnalytics(campaignId) {
        try {
            if (campaignId) {
                ...
                const deliveryStats = await this.campaignDB.campaign_delivery_summary.aggregate({
                    _sum: {
                        total_messages: true,
                        sent_count: true,
